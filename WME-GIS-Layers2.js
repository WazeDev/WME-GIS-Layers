...
(async function main() {
    ...
    // Function to build a mapping from 'country-subdivision' to 'name'
    ...
    // Helper object for country-subdivision name mapping
    const NameMapper = {
        // Convert full name to key ('country-subdivision')
        toKey(fullName) {
            // Attempt to find a matching key using the value from the mapping.
            return Object.entries(countrySubdivisionMapping)
                .find(([key, value]) => value === fullName)?.[0];
        },

        // Convert key ('country-subdivision') to full name
        toFullName(key) {
        ...
        // Get an array of keys ('country-subdivision')
        toKeyArray() {
    ...
    /**
     * The function `getMapExtent` converts map extent coordinates between WGS84 and Mercator
     * projections based on the specified projection type.
     * @param [projection=mercator] - The `projection` parameter in the `getMapExtent` function is used
     * to specify the type of projection for which you want to get the map extent. The function
     * supports two main projection types: Mercator and WGS84.
     * @returns The `getMapExtent` function returns the map extent coordinates based on the specified
     * projection type. If the projection is Mercator (e.g., 'mercator', '102100', 'EPSG:102100',
     * '3857', 'EPSG:3857'), it converts the WGS84 coordinates to Mercator projection using the
     * `turf.toMercator` function and returns the Merc
     */
    function getMapExtent(projection = 'mercator') {
        const wgs84Extent = sdk.Map.getMapExtent(); // Assume this gives WGS84 coordinates
        const wgs84LeftBottom = [wgs84Extent[0], wgs84Extent[1]];
        const wgs84RightTop = [wgs84Extent[2], wgs84Extent[3]];
    
        const mercatorProjections = ['mercator', '102100', 'EPSG:102100', '3857', 'EPSG:3857'];
        const wgs84Projections = ['wgs84', 'CRS84', '4326', 'EPSG:4326'];
    
        if (mercatorProjections.includes(projection.toLowerCase())) {
        ...
        } else if (wgs84Projections.includes(projection.toLowerCase())) {
    ...
    /**
     * Asynchronously fetches GIS feature data for layers visible within the current map viewport.
     *
     * The function executes the following operations:
     * - Clears layer labels if a popup is currently visible to reset any cached label data.
     * - Exits early based on certain conditions: whether data fetching should be ignored (`ignoreFetch`), or the map zoom level is below 12, in which case it filters layer checkboxes and returns without fetching.
     * - Calls `whatsInView()` to determine which layers are relevant based on the current visible map area.
     * - Initializes a `lastToken` object to manage cancellation and track fetched features and processed layers.
     * - Clears features that are not associated with `layersToFetch`, ensuring only mappings related to the layers intended for fetching are retained.
     * - Filters layer checkboxes to reflect currently fetched layers.
     * - Logs debug information on the layers being fetched and initiates fetching via XMLHttpRequests for each GIS layer in `layersToFetch`:
     *   - Constructs request URLs for feature data using `getUrl()` coupled with current map extent in Mercator projection.
     *   - Initiates GM_xmlhttpRequest for each layer to fetch its feature data.
     *   - Processes successful responses by calling `processFeatures()`.
     *   - Updates popup with layer labels if the popup is visible and all layers are processed.
     *   - Logs and handles HTTP errors accordingly.
     * 
     * The logic aims to dynamically manage data fetching based on visibility and zoom level parameters while tracking the fetched state.
     */
    async function fetchFeatures() {
        ...
        layersToFetch.forEach(gisLayer => {
            const url = getUrl(extentMercator, gisLayer);
            GM_xmlhttpRequest({
                url,
                context: lastToken,
                method: 'GET',
    ...
    async function onGisLayerToggleChanged() {
        ...
        if (checked) {
            const gisLayer = _gisLayers.find(l => l.id === layerId); //JS55CT
    ...
    function initLayersTab() {
        ...
        $('#panel-gis-subL1-layers').empty().append(
                ...
                : subL1.map(sub => $('<fieldset>', {
                    ...
                    $('<div>', {
                        ...
                        $('<div>', { class: 'controls-container', style: 'padding-top:0px;' }).append(
                            _gisLayers.filter(l => (l.countrySubL1 === sub && (!PRIVATE_LAYERS.hasOwnProperty(l.id)
    ...
    function initSettingsTab() {
        ...
        const createRadioBtn = (name, value, text, checked) => {
            const id = `${name}-${value}`;
            return [$('<input>', { type: 'radio', id, name, value }).prop('checked', checked),
                    $('<label>', { for: id }).text(text).css({ paddingLeft: '15px', marginRight: '4px' })];
        };
        ...
        $('#panel-gis-layers-settings').empty().append(
            $('<fieldset>', { style: 'border:1px solid silver;padding:8px;border-radius:4px;-webkit-padding-before:0;margin-top:-8px;' }).append(
                $('<legend>', { style: 'margin-bottom:0px;border-bottom:none;width:auto;' }).append(
                    $('<span>', { style: 'font-size:14px;font-weight:600;text-transform: uppercase;' }).text('Labels')
                ),
                $('<div>', { id: 'labelSettings' }).append(
                    $('<div>', { class: 'controls-container' }).css({ 'padding-top': '2px' }).append(
                        ...
                        $('<i>', {
                            class: 'waze-tooltip', id: 'gisAddrDisplayInfo',
                            'data-toggle': 'tooltip', style: 'margin-left:8px; font-size:12px',
                            'data-placement': 'bottom', title: `This may not work properly for all layers. Please report issues to ${SCRIPT_AUTHOR}.`
                        }).tooltip(),
                        $('<br>'),
        ...
        Object.keys(layersByCountry).sort().forEach(country => {
                ...
                $('<fieldset>', { style: 'border:1px solid silver;padding:8px;border-radius:4px;-webkit-padding-before:0;' }).append(
                    ...
                    $('<div>', { id: `country_${country}_body` }).append(
                        subRegions.map(countrySubL1 => {
                            ...
                            return $('<div>', { class: 'controls-container' }).css({ 'padding-top': '0px', display: 'block' }).append(
                                $('<input>', { type: 'checkbox', id, class: 'gis-layers-subL1-checkbox' }).change(countrySubL1, onSub1CheckChanged)
        ...
        $('#panel-gis-layers-settings').append(
            $('<fieldset>', { style: 'border:1px solid silver;padding:8px;border-radius:4px;-webkit-padding-before:0;' }).append(
                ...
                $('<div>', { class: 'controls-container' }).css({ 'padding-top': '2px' }).append(
                    $('<input>', { type: 'checkbox', id: 'fill-parcels' }).change(onFillParcelsCheckedChanged)
    ...
    /**
     * Asynchronously loads GIS layer data for visible countries and subdivisions from the map viewport.
     *
     * The function operates as follows:
     * - It first checks the current zoom level using `sdk.Map.getZoomLevel()`. If the zoom level is below 12, the function exits early, as loading for new areas is not required when the map is zoomed out.
     * - It invokes `whatsInView()`, a function expected to populate a global `_whatsInView` object with data about the countries and subdivisions currently visible on the map.
     * - Uniquely collects ISO_ALPHA3 country codes and subdivisions (subL1_id) into `countryCodes` and `regionCodes` sets from the `_whatsInView` data.
     *   - It iterates through countries and their subdivisions within the `_whatsInView` object, extracting the ISO_ALPHA3 codes and subdivision IDs needed for data loading.
     * - For each country code within `countryCodes`:
     *   - It determines the set of subdivision codes (`newRegionCodesToLoad`) that need loading.
     *   - Checks `alreadyLoadedCountries` to determine whether GIS layers for the country have been previously loaded. If not, it logs and schedules loading of all associated subdivisions.
     *   - Checks subdivisions against `alreadyLoadedSubL1` to skip reloading of already-processed subdivisions.
     *   - Calls `loadSpreadsheetAsync(isoCode, newRegionCodesToLoad)` to load layer data from the spreadsheet for this country and its subdivisions.
     *   - Calls `initGui(false)` to update the user interface after successful data loading.
     * - Tracks newly-loaded subdivisions by adding them to `alreadyLoadedSubL1`, preventing redundant loading in future calls.
     */
    async function loadVisibleCountryData() {
        if (sdk.Map.getZoomLevel() < 12) { // Only load new areas when the scripts Tab is active
            //filterLayerCheckboxes();
            return;
        }
        ...
        for (const isoCode of countryCodes) {
            ...
            newRegionCodesToLoad.forEach(regionCode => {
                ...
                logDebug(`Loaded GIS layers for subdivision ${regionCode}.`);
            });
        }
    }
    ...
    /**
     * Asynchronously loads GIS layer definitions from a Google Sheets document based on the provided ISO country code and region codes.
     * 
     * The function performs several tasks:
     * - Constructs the request URL using `LAYER_DEF_SPREADSHEET_URL`, a decoded API key, and the uppercase version of the ISO country code to access the correct sheet tab.
     * - Fetches JSON data from the Google Sheets using jQuery's `$.getJSON()`.
     * - Extracts version, field names, and layer definition rows from the returned data.
     * - Validates the current script version against the required version and ensures the necessary fields are present in the data.
     * - Constructs layer definitions for rows in the spreadsheet:
     *   - Iterates through each layer definition row, mapping field data to properties on the layer definition object.
     *   - Handles special cases for certain fields (e.g., splitting `subL2` and `labelFields`, compiling `processLabel`, casting `style` to predefined styles, evaluating `restrictTo` restrictions).
     *   - Constructs a unique `countrySubL1` identifier for layers based on country and subdivision.
     *   - Checks if each layer is enabled, allowed, and associated with valid visible subdivisions before adding it to `_gisLayers`.
     * - Logs debug information and potential errors for construction failures, specifically when `countrySubL1` cannot be constructed.
     * - Returns an object indicating success or any errors encountered during processing.
     *
     * @param {string} isoCode - The ISO code representing the country to fetch data for.
     * @param {Set} regionCodes - A set of subdivision codes that are currently visible and should be loaded.
     * @returns {Object} result - An object containing an error message if any validation fails or operations throw.
     */
    async function loadSpreadsheetAsync(isoCode, regionCodes) {
        ...
        const REQUIRED_FIELD_NAMES = [
            'country', 'subL1', 'name', 'id', 'subL2', 'url', 'where', 'labelFields',
